...
const CardTrack = (prop) => {
    ...
    const scrollRangeStart = windowHeight + buffer;
    const scrollRangeEnd = windowHeight * 3 - buffer;
    
    const scrollPercent = (() => {
        if (scrollY < scrollRangeStart) return 0;
        if (scrollY > scrollRangeEnd) return 100;
        return ((scrollY - scrollRangeStart) / (scrollRangeEnd - scrollRangeStart)) * 100;
    })();

    const zoomedObjectPosition = `${Math.min(scrollPercent, 100)}%`;
    const trackScrollPosition = `-${scrollPercent}%`;
